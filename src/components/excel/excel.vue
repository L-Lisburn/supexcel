  ...
  <div class="exl-container">
    <div class="exl-tool-bar">
      <div class="exl-tool-font">
        ...
        <br />
        <button @click="_exl_font('bold',1)">
          <i class="icon excel-icons">&#xe617;</i>
        </button>
        <button @click="_exl_font('italic',2)">
          <i class="icon excel-icons">&#xe693;</i>
        </button>
        <button @click="_exl_font('underline',3)">
          <i class="icon excel-icons">&#xe6fa;</i>
        </button>
        <button @click="_exl_font('strikethrough',4)">
          <i class="icon excel-icons">&#xe6fb;</i>
        </button>
        <button>
          <i class="icon excel-icons">&#xe682;</i>
      ...
      <div class="exl-tool-views">
        ...
        <input
          type="text"
          v-model="exchange"
          placeholder="交换列或者行请以英文逗号隔开，交换多组请以分号隔开，例：a1,a3;1,3"
        />
      </div>
    </div>
    <div class="exl-content">
      <table class="exl-table" id="tableWidth">
        <thead v-if="columns !== undefined" class="exl-thead">
          ...
          <tr v-for="(columns, index) in columns" :key="index">
            <td class="exl-row-pattern" v-if="pattern">
              {{ index + 1 }}
            </td>
            <td
        ...
        <tbody>
          <tr v-for="(item, index) in data" :key="index">
            ...
            >
              <span class="exl-col-name"
                :class="{'bold':bold,'italic':italic}"
              >
              {{ item.value }}
              </span>
            </td>
...
export default {
  ...
  methods: {
    ...
    _exl_font(state, y) {
      ...
      //获取整个文本
      var text = document.getElementsByClassName("exl_active");
    //   console.log(text);
      // console.log(text[0].children);
      ...
      // console.log(button[1]);
       ...
       if (this.font.indexOf(state) !== -1) {
           console.log(1111);
          this.font.splice(this.font.indexOf(state), 1)
        ...
        //   /* 下划线 */
          case "underline":
            this.font.push(state);
        //     for (var i in text) {
        //     //   text[i].children.style.textDecoration = "underline";
        //     }
        ...
        // console.log(this.font);
         ...
         for (var j in this.font) {
              if (this.font[j] == state) {
                ...
                for(var i in text){
                   text[i].style.fontWeight=state; 
                   text[i].style.fontStyle=state;
